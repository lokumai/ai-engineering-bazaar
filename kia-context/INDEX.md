...
| `PROGRESS_2.md` | **Part 2, and the ACTIVE part** — a new milestone is appended here. M16: the interface rebuilt on the design language, in ten stages; M17: three listings folded into one; M18: the home page, the bar and the two ends of a module | **All ten stages shipped.** The stage table, every stage's brief, and seven reports — each of which records what its own brief had wrong, so **read a stage's report before trusting its brief**. It closes with the capability ledger — one row per capability, where it lives, and the behavioural test that proves it survived — then with the review pass: 37 findings, **13 confirmed defects inside a green gate**, and eight more left deliberately as the author's call — read that before quoting any figure from this file — then **M17, the curriculum fold — SHIPPED**, whose report names five things its own brief had wrong, including a breadcrumb that pointed every module page at a retired route with nothing able to fail; then **M18, the front door and the chrome — SHIPPED**, whose report names three defects a screenshot found and no test could. **Every milestone here states its deliverables and acceptance criteria as `- [ ]` checkboxes, ticked as the work lands, and closes with a dated report** — the loop that governs that lives in part 1 and is linked from the top of part 2. Then **M20, the catalog's descriptions — SHIPPED**, whose report names five things measured rather than assumed, four of them found by screenshot and one by probing a browser, and none of them visible to 1,138 green tests. Then **M21, the module page and the rail — SHIPPED**, whose report records a one-pixel overflow that a fold latched and a restore could not undo, and a review that caught a regression inside the fix itself. Then **M22, the progress page redesigned — SHIPPED**, whose report records that eight of the ten routes drawing a trail named their page differently from its own heading, and a review that pointed the project's own rule 16 back at the milestone. Then **M19, the second language — SHIPPED less its chrome**: 19 Turkish module pages at `/tr/`, whose report records two capabilities that were being dropped on every one of them in silence because a matcher was written in one language. It closes with **M23, written and not started** — the interface's own strings, which are authored content in the author's language rather than code. |
...
`M1…M15` are milestones in `logs/PROGRESS.md` and `M16…M23` are in `logs/PROGRESS_2.md`. `D1…D25` are decisions in `logs/BRAINSTORM.md` and `D26…D70` are in `logs/BRAINSTORM_2.md`, where `O1…O4` are the open questions
...
runs straight through — `M1…M23`, `D1…D70` — so a citation never has to say
