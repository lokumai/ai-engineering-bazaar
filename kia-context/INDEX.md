...
`M1…M15` are milestones in `logs/PROGRESS.md` and `M16…M22` are in `logs/PROGRESS_2.md`. `D1…D25` are decisions in `logs/BRAINSTORM.md` and `D26…D69` are in `logs/BRAINSTORM_2.md`, where `O1…O4` are the open questions
...
runs straight through — `M1…M22`, `D1…D69` — so a citation never has to say
