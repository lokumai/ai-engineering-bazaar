...
> **Sidebar — Hermes Agent, and why it sits in a different category.** `NousResearch/hermes-agent` (MIT) does real coding work: `hermes chat --worktree --checkpoints` gives you a git worktree per task, and `hermes kanban` is a shipped multi-agent work queue. But its own docs draw the line for us — *"It's **not a coding copilot tethered to an IDE** or a chatbot wrapper around a single API. It's an **autonomous agent** that gets more capable the longer it runs"* ([Hermes Agent docs](https://hermes-agent.nousresearch.com/docs/)). It reaches you over Telegram, Discord, Slack or email, keeps persistent memory across sessions, and runs on a VPS you never SSH into. That makes its home nightly, cross-repo, scheduled work rather than "refactor this file" — so it earns a full treatment in [Module 15: Personal Agents](15_personal_agents.md), not a row in the table above. The category boundary is about *where the agent lives and who triggers it*, not about whether it can write code.
