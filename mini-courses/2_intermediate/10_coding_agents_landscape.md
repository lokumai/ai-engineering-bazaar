...
**Hermes Agent** — `NousResearch/hermes-agent`, MIT, installed with `curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash`. It does real engineering work: `hermes chat --worktree --checkpoints` gives you a git worktree per task with rollback, `hermes project` manages deterministic per-repo workspaces, and `hermes kanban dispatch` is a shipped multi-agent work queue. It runs any provider — Nous Portal, OpenRouter, OpenAI, your own endpoint — and ships `hermes claw migrate` for people arriving from OpenClaw ([Hermes Agent docs, 2026-08-25](https://hermes-agent.nousresearch.com/docs/)).

What makes it different from the three above is not capability but **shape**, and its own docs say so plainly: *"It's **not a coding copilot tethered to an IDE** or a chatbot wrapper around a single API. It's an **autonomous agent** that gets more capable the longer it runs."* You reach it from Telegram, Discord, Slack, WhatsApp, Signal or email rather than a terminal you are sitting in front of; it keeps persistent memory and auto-generates skills across sessions; it has a scheduler; and it expects to live on a VPS you never SSH into. So its natural job is *"every night, check CI on all six repos and open issues for what broke"* — not *"refactor this file while I watch"*.

**Pick it when** your work is recurring, cross-repo and unattended, and you want one agent with memory rather than a fresh session each time. **Don't pick it** as your interactive pair-programmer; one of the three above will feel better at that. Full treatment — including the security consequences of an always-on agent holding your credentials — is [Module 15: Personal Agents](15_personal_agents.md).
...
- [Hermes Agent docs](https://hermes-agent.nousresearch.com/docs/) — Nous Research, fetched 2026-08-25. Its own "What is Hermes Agent?" section is the cleanest statement of the personal-agent-versus-coding-agent boundary.
