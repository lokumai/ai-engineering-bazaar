...
| **What it can do per step** | Tool surface and permissions | [Harness Engineering](harness_engineering.md)'s territory. The loop-level consequence: parallel tool calls cut wall-clock hard — Anthropic reports *"up to 90%"* off research time ([Anthropic, 2025-06-13](https://www.anthropic.com/engineering/multi-agent-research-system)) |
| **What it remembers** | Context carried across iterations | [Context Engineering](context_engineering.md)'s territory. The loop-level lever is subagent isolation: delegate, and only a distilled report comes back |
