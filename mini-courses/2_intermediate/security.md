...
- **[Microsoft's PyRIT](https://github.com/microsoft/PyRIT)**, the Python Risk Identification Tool for generative AI, is the framework to reach for when you want to write the attacks yourself instead of running someone else's list. You assemble a run out of four pieces: the target you are attacking, converters that transform the prompt, the attack that drives the conversation, and scorers that judge what came back. What earns it a place here is that the attacks named earlier in this module arrive as classes you can run, so `CrescendoAttack`, `TreeOfAttacksWithPruningAttack`, `PAIRAttack`, `SkeletonKeyAttack` and `ManyShotJailbreakAttack` are each one import away, and since version 1.0 there is also `pyrit_scan`, which runs a scripted scenario in a single command and is the shape you want in CI. Two things will save you an hour if you go reading about it: `Orchestrator` was renamed to `Attack` in December 2025, so any tutorial older than that will not run, and the old `Azure/PyRIT` repository is archived rather than redirected, so a stale link still serves a page that looks current.
...
- [PyRIT](https://github.com/microsoft/PyRIT): Microsoft's red-teaming framework, with Crescendo, TAP and PAIR as classes you can run
