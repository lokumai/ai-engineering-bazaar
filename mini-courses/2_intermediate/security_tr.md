...
- **[Microsoft'un PyRIT'i](https://github.com/microsoft/PyRIT)**, yani generative AI için Python Risk Identification Tool, saldırıları başkasının listesini çalıştırmak yerine kendin yazmak istediğinde uzanacağın framework. Bir çalışmayı dört parçadan kuruyorsun: saldırdığın target, prompt'u dönüştüren converter'lar, konuşmayı yürüten attack, ve geri geleni notlayan scorer'lar. Buraya girmeyi hak etmesinin sebebi, bu modülde daha önce adı geçen saldırıların çalıştırabileceğin sınıflar olarak gelmesi: `CrescendoAttack`, `TreeOfAttacksWithPruningAttack`, `PAIRAttack`, `SkeletonKeyAttack` ve `ManyShotJailbreakAttack` her biri tek bir import uzakta, ve 1.0'dan beri bir de `pyrit_scan` var; senaryolu bir çalışmayı tek komutla yürütüyor, ki CI'da isteyeceğin şekil de bu. Hakkında okumaya başlarsan bir saatini kurtaracak iki şey: `Orchestrator`, Aralık 2025'te `Attack` olarak yeniden adlandırıldı, dolayısıyla ondan eski bir tutorial çalışmayacak, ve eski `Azure/PyRIT` deposu yönlendirilmiş değil arşivlenmiş, yani eski bir link hâlâ güncel görünen bir sayfa veriyor.
...
- [PyRIT](https://github.com/microsoft/PyRIT): Microsoft'un red-teaming framework'ü, Crescendo, TAP ve PAIR çalıştırabileceğin sınıflar olarak
