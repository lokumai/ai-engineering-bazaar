 ...
 *    has written.
 *
 * A step names a sheet by SLUG and by nothing else (§12.1.3). It carries no
 * module number, because a number is a position the corpus computes and a copy
 * kept here would only be one more thing to fall out of step with it.
 *
...
/**
 * Whether the sheet a step points at is drawn.
 *
 * **The caller supplies the answer, and that is the whole point.** This used to
 * be `step.module <= 15`, a number written here standing in for the corpus's own
 * `status: ready`, and it was already wrong: sheet 15 is a draft, so every
 * denominator on `/path/` was one too many the moment a path listed it. Which
 * sheets are drawn is a fact only `lib/content/` can measure, and `lib/path/`
 * imports nothing (§12.2), so it arrives as an argument. Omitting it is a
 * compile error rather than a constant nobody notices going stale.
 */
export function isDrawnStep(step: PathStep, drawn: ReadonlySet<string>): boolean {
  return drawn.has(step.slug)
}

/** §13.4.2 — the denominator. Drawn steps only, never the whole list. */
export function drawnCount(path: LearningPath, drawn: ReadonlySet<string>): number {
  return path.steps.filter((step) => isDrawnStep(step, drawn)).length
}
