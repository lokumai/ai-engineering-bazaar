...
const rendered = new Map(
  await Promise.all(
    modules.map(
      // `sheet` is not optional any more, and this file is why the throw that
      // makes it mandatory is worth having: nothing here asserts anything about
      // links or figure numbers, so a sourceless render read as harmless — and
      // a sourceless render is precisely what left four dead cross-references
      // in the published summary panels. A render that cannot resolve a
      // relative `.md` href now says so instead of shipping it.
      async (module) =>
        [
          module.slug,
          await renderMarkdown(module.body, { sheet: module.frontmatter.module }),
        ] as const,
    ),
  ),
)
