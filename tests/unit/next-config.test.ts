...
async function loadConfig(basePath: string | undefined): Promise<StaticExportConfig> {
  if (basePath === undefined) delete process.env.SITE_BASE_PATH
  else process.env.SITE_BASE_PATH = basePath
  vi.resetModules()
...
describe('next.config.mjs', () => {
  ...
  beforeEach(() => { delete process.env.SITE_BASE_PATH })
  afterEach(() => { process.env.SITE_BASE_PATH = original })
  ...
  it('applies the deploy base path to both routes and assets', async () => {
    const config = await loadConfig('/ai-engineering-bazaar')
    expect(config.basePath).toBe('/ai-engineering-bazaar')
    expect(config.assetPrefix).toBe('/ai-engineering-bazaar')
  })
