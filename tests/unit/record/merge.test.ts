...
describe('prefs — local wins (§14.7.2)', () => {
  it('keeps the device preference', () => {
    const local = record({ prefs: { charKeys: false, railFolded: false, aliasNamedFor: null, catalogView: null } })
    const remote = record({ prefs: { charKeys: true, railFolded: false, aliasNamedFor: null, catalogView: null }, days: ['2026-08-01'] })
...
describe('the properties §14.2.3 relies on', () => {
  ...
  it('is commutative on the modules even when the asymmetric rows differ', () => {
    const skewed = record({
      ...
      prefs: { charKeys: false, railFolded: false, aliasNamedFor: null, catalogView: null },
    })
